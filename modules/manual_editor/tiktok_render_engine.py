...
def convert_frame_to_portrait(frame, target_w=1080, target_h=1920):
    """
    Chuyển đổi khung hình ngang sang dọc 9:16 có nền mờ làm mờ
    """
    h, w = frame.shape[:2]
    
    # Nếu video đã là dạng dọc sẵn, chỉ cần resize về đúng chuẩn
    if w / h < 0.8:
        return cv2.resize(frame, (target_w, target_h), interpolation=cv2.INTER_LINEAR)
        
    # 1. Tạo nền mờ (Background) phóng to từ video gốc
    # Đóng gói nhỏ khung hình để làm mờ cực nhanh, tránh suy giảm hiệu năng render
    bg_small = cv2.resize(frame, (108, 192), interpolation=cv2.INTER_LINEAR)
    bg_blurred = cv2.GaussianBlur(bg_small, (15, 15), 0)
    bg = cv2.resize(bg_blurred, (target_w, target_h), interpolation=cv2.INTER_LINEAR)
    
    # 2. Bo tỷ lệ video gốc (Foreground) khớp chiều rộng 1080
    fg_w = target_w
    ...
    # 3. Đè video gốc vào chính giữa nền mờ
    y_offset = (target_h - fg_h) // 2
    bg[y_offset:y_offset+fg_h, 0:fg_w] = fg
    
    return bg

def render_tiktok_video(input_path, blur_boxes, texts, width=1080, height=1920, custom_output_dir=None,
                        speed=1.0, strip_metadata=False, split_video=False,
                        font_size=30, text_color=(255, 255, 0)):
    """
    Xử lý video từng frame bằng OpenCV để chuyển đổi sang dọc 9:16, làm mờ logo và chèn chữ,
    ...
    temp_video = os.path.join(output_dir, f"temp_tiktok_{filename}")
    final_output = os.path.join(output_dir, f"TIKTOK_EDIT_{filename}")
    
    cap = cv2.VideoCapture(input_path)
    fps = cap.get(cv2.CAP_PROP_FPS)
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    
    # Kích thước cố định cho dọc là 1080x1920
    ...
    print(f"🎬 Bắt đầu Render TikTok (Dọc): {filename}")
    frame_count = 0
    ...
    while cap.isOpened():
        ...
        # 0. CHUYỂN ĐỔI NGANG SANG DỌC KÈM BLUR NỀN
        frame = convert_frame_to_portrait(frame, width, height)
        ...
        # 2. XỬ LÝ CHÈN CHỮ (TEXT) BẰNG PIL (Hỗ trợ Tiếng Việt)
        if texts:
            current_time_sec = frame_count / fps if fps else 0
            # Lọc các chữ cần vẽ trong frame này trước
            active_texts = [
        ...
        out.write(frame)
        if frame_count % 30 == 0:
            print(f"   Tiến độ Render TikTok: {frame_count}/{total_frames} frames")
        ...
        if split_video:
            ...
            try:
                ...
                if out_fps > 0:
                    ...
                    # Lấy tên phim không chứa TIKTOK_EDIT_ hoặc MANUAL_EDIT_
                    base_name = filename.replace("TIKTOK_EDIT_", "").replace("MANUAL_EDIT_", "")
