...
# Import engine render (sẽ tạo sau)
try:
    from manual_editor.render_engine import render_manual_video
except ImportError:
    render_manual_video = None
...
class ManualEditorApp(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.blur_boxes = [] # Danh sách [(x1, y1, x2, y2)] tọa độ gốc
        self.texts = []      # Danh sách [{"text": "abc", "x": x, "y": y, "voice": True, "start": 0, "end": 9999}]
    ...
    def save_settings(self):
        ...
        try:
            ...
            self.lbl_status.configure(text="✅ Đã lưu cấu hình biên tập!", text_color=GRN)
        except Exception as e:
    ...
    def _build_left_panel(self):
        # Sử dụng CTkScrollableFrame để hiển thị được đầy đủ và trực quan mọi cấu hình
        ...
        ctk.CTkLabel(self.left_panel, text="🎬 EDIT & LÁCH BẢN QUYỀN YT", font=F(16, "bold"), text_color=ORG).pack(pady=(20, 10))
        
        # Nút Tải Video (Hỗ trợ full các định dạng video phổ biến)
        ...
        # Thiết lập màu sắc hiển thị phù hợp
        contrast = "#1E1E2E" if (self.text_color_rgb[0]*0.299 + self.text_color_rgb[1]*0.587 + self.text_color_rgb[2]*0.114) > 186 else "#CDD6F4"
    ...
    def _build_right_panel(self):
        ...
        lbl_hint = ctk.CTkLabel(self.right_panel, text="MÀN HÌNH PREVIEW KÉO THẢ (PLAY/PAUSE)", font=F(14, "bold"), text_color=T3)
    ...
    def update_video_loop(self):
        if self.is_playing and self.cap:
            self.read_and_show_frame()
            # Tự động loop lại từ đầu nếu hết video
            if self.current_frame_idx >= self.total_frames - 1:
                self.current_frame_idx = 0
                self.cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
            # Tính thời gian delay theo fps
    ...
    def update_canvas(self):
        ...
        # Lấy kích thước hiện tại của khung Canvas
    ...
    def redraw_overlays(self):
        ...
        # Vẽ Text (Chỉ hiển thị nếu thời gian hiện tại nằm trong start/end)
        ...
        for idx, txt in enumerate(self.texts):
            ...
            # Tạo hiệu ứng Viền Đen (Stroke) như CapCut / Tool Automation
        ...
        # Gán sự kiện Click đúp trực tiếp vào các dòng chữ
    ...
    def on_text_double_click(self, event):
        # Lấy ID của item text vừa bị click đúp
    ...
    def on_double_click(self, event):
        # Sự kiện click đúp trên nền Canvas trống (không trúng chữ)
        pass
    ...
    def on_press(self, event):
        ...
        # Kiểm tra xem có click trúng Phụ đề nào không để cho phép Kéo-Thả di chuyển (Drag & Drop)
        ...
        for idx in range(len(self.texts)-1, -1, -1):
            ...
            # Click trong phạm vi bounding box (chữ đang canh giữa center)
            if abs(txt["x"] - real_x) < 250 and abs(txt["y"] - real_y) < 50:
                self.dragging_text_idx = idx
                return # Trúng chữ rồi thì không kích hoạt công cụ vẽ Blur hay Text nữa
        ...
        if self.current_tool == "blur":
            ...
            if text_content:
                self.texts.append({
                    ...
                    "start": 0, # Text thủ công hiện từ đầu đến cuối
                    "end": 999999
                })
                self.redraw_overlays()
                self.update_layer_panel()
                self.text_input.delete(0, tk.END) # Clear after placing

    def on_drag(self, event):
        if not self.video_path: return
        
        if self.dragging_text_idx is not None:
            # Đang kéo thả Text, cập nhật tọa độ liên tục
    ...
    def on_release(self, event):
        ...
        if self.current_tool == "blur" and self.temp_rect:
            ...
            if rx2 - rx1 > 10 and ry2 - ry1 > 10: # Bỏ qua click lầm
    ...
    def _run_auto_sub_thread(self):
        try:
            ...
            temp_audio = os.path.join(tempfile.gettempdir(), "temp_manual_audio.mp3")
            temp_srt = os.path.join(tempfile.gettempdir(), "temp_manual_auto.srt")
            temp_translated = os.path.join(tempfile.gettempdir(), "temp_manual_translated.srt")
            ...
            for sub in subs:
                self.texts.append({
                    "text": sub["text"],
                    "x": int(self.video_width / 2) - 150,  # Center-ish
                    "y": int(self.video_height * 0.8),    # Bottom-ish
                    "voice": self.voice_var.get(),
                    "start": sub["start"],
                    "end": sub["end"]
                })
            ...
            # Tự động nhảy đến vị trí có phụ đề đầu tiên để Sếp thấy ngay!
    ...
    def render_video(self):
        ...
        if render_manual_video is None:
            messagebox.showerror("Lỗi", "Chưa liên kết Module Render!")
            return
        ...
        # Get settings values safely in main GUI thread
    ...
    def _run_render_thread(self, speed_val, strip_metadata, split_video, font_size, text_color_rgb):
        try:
            output_path = render_manual_video(
