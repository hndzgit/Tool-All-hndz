...
class TrimmerApp(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(f_config, text="⏱ Số giây muốn lấy đầu tiên:", text_color=T2, font=ctk.CTkFont(size=14, weight="bold")).pack(side="left")
        self.e_duration = ctk.CTkEntry(f_config, width=60)
        self.e_duration.insert(0, "59")
        self.e_duration.pack(side="left", padx=10)
        ctk.CTkLabel(f_config, text="(Khuyên dùng 59s cho YouTube Shorts/TikTok)", text_color=T3).pack(side="left")

        # --- TÙY CHỌN 9:16 ---
        f_options = ctk.CTkFrame(self, fg_color="transparent")
        f_options.pack(fill="x", padx=40, pady=(0, 10))
        
        self.chk_vertical_var = ctk.StringVar(value="off")
        self.chk_vertical = ctk.CTkCheckBox(f_options, text="Chuyển video ngang thành Dọc 9:16 (Làm mờ nền đen)", 
                                            variable=self.chk_vertical_var, onvalue="on", offvalue="off",
                                            text_color="#FCA5A5", font=ctk.CTkFont(size=13, weight="bold"))
        self.chk_vertical.pack(side="left")

        # --- ACTIONS ---
        f_actions = ctk.CTkFrame(self, fg_color="transparent")
        f_actions.pack(pady=20)
        
        self.btn_start = ctk.CTkButton(f_actions, text="✂️ BẮT ĐẦU CẮT HÀNG LOẠT", font=ctk.CTkFont(size=16, weight="bold"), height=50, width=250, fg_color=ORG, hover_color="#D97706", command=self.start_cutting)
        self.btn_start.pack(side="left", padx=10)
        
        self.btn_stop = ctk.CTkButton(f_actions, text="🛑 DỪNG", font=ctk.CTkFont(size=16, weight="bold"), height=50, width=120, fg_color=RED, hover_color="#CC0000", command=self.stop_cutting, state="disabled")
        self.btn_stop.pack(side="left", padx=10)

        # --- LOGS ---
        self.logs = ctk.CTkTextbox(self, font=ctk.CTkFont("Consolas", 12), text_color="#A3E635", fg_color="#0D1117", corner_radius=10, border_width=1, border_color="#30363D")
        self.logs.pack(fill="both", expand=True, padx=40, pady=(0, 20))
        self.logs.configure(state="disabled")

        info_text = "💡 Lưu ý: Hệ thống cắt lõi trực tiếp bằng FFMPEG không qua render hình ảnh.\nTốc độ cắt 1 giây/video, Giữ nguyên 100% độ sắc nét và khung hình của video gốc."
        self.log(info_text)
    ...
    def load_files(self):
        files = filedialog.askopenfilenames(title="Chọn Các Video Cần Cắt", filetypes=[("Video Files", "*.mp4 *.mov *.avi *.mkv")])
        if not files: return
    ...
    def choose_output_dir(self):
        folder = filedialog.askdirectory(title="Chọn Thư Mục Lưu Video Đã Cắt")
        if folder:
    ...
    def stop_cutting(self):
        if self.is_running:
            self.log("🛑 Đang huỷ tiến trình cắt...")
            self.stop_event.set()
    ...
    def start_cutting(self):
        ...
        try:
            duration = int(self.e_duration.get().strip())
        except ValueError:
            self.log("⚠️ Vui lòng nhập số giây (VD: 59) hợp lệ!")
            return
        ...
        threading.Thread(target=self.cutter_worker, args=(duration, self.chk_vertical_var.get() == "on"), daemon=True).start()

    def cutter_worker(self, duration, is_vertical):
        success = 0
        total = len(self.video_paths)
        
        if self.output_dir:
            self.log(f"\n🚀 BẮT ĐẦU CẮT LẤY {duration}s ĐẦU TIÊN CỦA {total} VIDEO...")
        ...
        for idx, input_path in enumerate(self.video_paths):
            ...
            if self.output_dir:
                ...
                out_folder = os.path.join(os.path.dirname(input_path), f"Shorts_{duration}s")
            ...
            self.log(f"[{idx+1}/{total}] ✂️ Đang phay: {v_name}")
            
            if is_vertical:
                # Nếu yêu cầu làm mờ viền dọc 9:16, bắt buộc phải render lại hình ảnh.
                # Sử dụng Apple VideoToolbox HW Accel để tốc độ render nhanh nhất trên Mac.
                import platform
                ...
                self.log(f"   => Đang render nền mờ 9:16 (sẽ tốn thêm vài giây...)")
            else:
                # Cắt copy thô 1 giây
                cmd = [
                    "ffmpeg", "-y", "-i", input_path,
            ...
            try:
                # Chạy ngầm, không in rác ra stdout
                result = subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=30)
                if result.returncode != 0 and is_vertical and codec == "h264_videotoolbox":
                    ...
                    try:
                        ...
                        pass
                    result = subprocess.run(fallback_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=30)
        ...
        self.log(f"\n✅ HOÀN THÀNH: Đã cắt thành công {success}/{total} Videos!")
